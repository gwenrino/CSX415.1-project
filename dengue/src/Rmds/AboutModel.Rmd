...
withseason$currentstaff <- case_when(
...
For instructions on installing and using the tool, please see ().  
...
The second type of information is recent number of cases of dengue disease. This is also intuitive; you would not expect to see a precipitous increase or decrease in number of cases from one week to the next most of the time; today's incidence of disease is related to tomorrow's. Our mathematical model takes advantage of this strong predictive pattern (a phenomenon called autocorrelation).  
...
It should be emphasized that the forecasting tool gives much more dependable results for near-future predictions than long-term ones; the uncertainty rises dramatically for longer time horizons. The average error (positive or negative) in predicted number of dengue cases looking ahead one week is just 1.06; looking ahead six weeks the error rises to 7.98; looking ahead six months the error is 18.03. (For more information about the accuracy of the model at different time horizons, please see the document 4.DynRegEval.pdf, which can be found at dengue/reports/ModelEvaluations/4.DynRegEval.pdf)
...
The error in the current staffing model cost the Departamento de Salud 500 dollars in the past six weeks.

```{r, echo=FALSE}
WeekOf <- dengue$week_start_date[931:936]
kable(data.frame(WeekOf,ActualCases,RequiredStaff,CurrentStaff,CurrentError,CostCurrentError))
...
Had it been in use during this time period, the forecasting tool would have provided the Departamento de Salud a savings of 500 dollars over the current staffing model.

```{r, echo=FALSE}

kable(data.frame(WeekOf,ActualCases,RequiredStaff,RecommendedStaff,RecError,CostRecError))
```
...
The table below shows the current staffing model's performance during this six-week period, with a total cost from over- and underestimating staffing needs of 5,600 dollars.
...
The table below shows the forecasting tool's performance during the same six-week period, with a total cost from over- and underestimating staffing needs of 3,600 dollars.

Had it been in use during this time period, the forecasting tool would have provided the Departamento de Salud a savings of 2,000 dollars over the current staffing model.
...
In each of the three examples, the forecasting tool performed better than the current staffing model, saving signficant resources (500 - 13,300 dollars over six weeks) that the Departamento de Salud could have directed to other public health projects, including education, vector management, and research.
