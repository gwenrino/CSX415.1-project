...
nonres_guests.scatter <- ggplot(dengue) +
...
# Mean cases by weekofyear
dengue %>% group_by(weekofyear) %>% 
  summarize(mean = mean(total_cases)) %>%
  ggplot(aes(x=weekofyear, y=mean)) +
  geom_col() + 
  labs(title = "Mean Dengue Cases by Week of the Year", x = "Week of Year", y = "Mean Cases")
ggsave(file.path('graphs/EDAviz', 'weekofyear.mean.pdf'))
